 ...
 *
 * Device-only. Owns the real driver handles and supplies the real bodies of
 * sensor_hub's three read seams (sensor_hub_light_read/temp_read/level_read
 * — declared in lib/sensor_hub/include/sensor_hub.h, called from
 ...
 *
 * Known bench-verification risk (flagged, not resolved here): src/main.c's
 * Phase 1/2 one-shot boot read independently creates its own I2C bus on
 * I2C_NUM_0 and configures the same level-switch GPIOs. Per this phase's
 * instructions that boot-read code stays intact ("extend, don't replace"),
 * so both it and this init run during boot. Whether a second
 * i2c_new_master_bus() call on the same already-installed port at least
 * fails cleanly (rather than corrupting driver state) is an open item for
 * Phase 3 hardware verification, not something host-testing can confirm.
 */
static void sensor_drivers_init(void) {
...
static void sampler_task(void *arg) {
    ...
    for (;;) {
        /* TWDT subscription brackets ONLY the read window (~500 ms total
         * across the three sensors), never the CONFIG_HYDRO_SAMPLE_INTERVAL_SEC
         * sleep below. A permanently-subscribed task would blow the
         * (default 5 s) watchdog on its very first sleep — see AC-ASYNC-2
         * in the task file for the full rationale. */
        ...
        vTaskDelay(pdMS_TO_TICKS((uint32_t)CONFIG_HYDRO_SAMPLE_INTERVAL_SEC * 1000U));
    }
}

void sampler_start(void) {
    sensor_drivers_init();
    xTaskCreatePinnedToCore(sampler_task, "sampler", SAMPLER_TASK_STACK_SIZE, NULL,
                             SAMPLER_TASK_PRIORITY, NULL, tskNO_AFFINITY);
}
